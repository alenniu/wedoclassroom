...
const Announcement = ({_class, announcement={}, assignment=null, user={}}) => {
    const {message, createdAt} = announcement;
    const {description} = (assignment || {});
    const title = announcement.title || (assignment && `${user.name?.first} posted a new assignment: `) || `${user.name?.first}`
    return (
        <div className='announcement-container'>
            <div className='user-image-container'>
                <img src={get_full_image_url(_class.cover_image || user.photo_url)} />
            </div>
