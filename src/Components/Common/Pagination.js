...
const Pagination = ({onChangePage, onChangePerPage, total=0, pages=1, page=1, add1page=true, perPageOptions=[20], perPage=20, perPageLabel}) => {
    ...
    return (
        <div className='pagination-container'>
            <span className={`prev clickable ${can_go_back?"":"disabled"}`} onClick={onClickPrevPage}><RiArrowLeftSLine size={25} /></span>
            <span className={`next clickable ${can_go_forward?"":"disabled"}`} onClick={onClickNextPage}><RiArrowRightSLine size={25} /></span>
            ...
            <div className='input-container select pagination per-page'>
                <Select disabled={perPageOptions.length < 2} value={perPage} onChange={onChangePerPageOption}>
