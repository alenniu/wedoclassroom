...
async function create_user({name, email, phone, password, gender, school, grade, date_enrolled, birth=Date.now(), type, role="", emergency_contact, credits=0}, admin_user=null){
    try{
        if((type !== "student") || (admin_user && mongoose.isValidObjectId(admin_user._id))){
            if(validate_email(email) && validate_password(password) && birth && type && name.first && name.last){
                ...
                const new_user = await (new User({name, email, phone, password: hashed_password, gender, school, grade, date_enrolled: new Date(date_enrolled), birth: new Date(birth), type, emergency_contact, activated: false, role, created_by: admin_user, credits})).save();
